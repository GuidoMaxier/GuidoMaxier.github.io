...
async function cargarCanales(serverId) {
  try {
      // Limpiar la lista de canales existentes
      channelList.innerHTML = '';

      const response = await fetch(`http://127.0.0.1:5000/canal/server/${serverId}`, {
          method: 'GET',
          credentials: 'include',
      });
  ...
  async function crearCanal(canalData) {
    try {
        console.log(canalData);

        // Realiza la solicitud HTTP POST al backend
        const response = await fetch('http://127.0.0.1:5000/canal/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        // Aquí puedes realizar cualquier otra acción después de crear el canal con éxito
    } catch (error) {
        console.error(error.message); // Maneja errores si ocurren
    }
}
